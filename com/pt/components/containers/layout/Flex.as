    ...
    public class Flex
    {
            ...
            h:Number):Number
        {
            ...
            for(i = 0; i < n; i++)
            {
                ...
                {
                    var percentWidth:Number = IUIComponent(child).percentWidth;
                    var percentHeight:Number = IUIComponent(child).percentHeight;
                    var height:Number;
                    
                    if(!isNaN(percentHeight) && IUIComponent(child).includeInLayout)
                    {
                        height = Math.max(IUIComponent(child).minHeight,
                                          Math.min(IUIComponent(child).maxHeight,
                                                   ((percentHeight >= 100) ? h : h * percentHeight / 100)));
                    }
                    else
                    {
                        height = IUIComponent(child).getExplicitOrMeasuredHeight();
                    }
                    
                    if(!isNaN(percentWidth) && IUIComponent(child).includeInLayout)
                    {
                        totalPercentWidth += percentWidth;
                        
                        childInfo = new FlexChildInfo();
                        childInfo.percent = percentWidth;
                        childInfo.min = IUIComponent(child).minWidth;
                        childInfo.max = IUIComponent(child).maxWidth;
                        childInfo.height = height;
                        childInfo.child = IUIComponent(child);
                        
                        childInfoArray.push(childInfo);
                    }
                    else
                    {
                        var width:Number = IUIComponent(child).getExplicitOrMeasuredWidth();
                        ...
                        {
                            IUIComponent(child).setActualSize(Math.floor(width),
                                                              Math.floor(height));
                        }
                        else
                        {
                            IUIComponent(child).setActualSize(width, height);
                        }
            ...
            if(totalPercentWidth)
            {
                ...
                for(i = 0; i < n; i++)
                {
                    ...
                    child = DisplayObject(childInfo.child);
                    
                    // if scaled and zoom is playing, best to let the sizes be non-integer
                    // otherwise the rounding creates an error that accumulates in some components like List
                    if(child.scaleX == 1 && child.scaleY == 1)
                    {
                        IUIComponent(child).setActualSize(Math.floor(childInfo.size),
                                                          Math.floor(childInfo.height));
                    }
                    else
                    {
                        IUIComponent(child).setActualSize(childInfo.size, childInfo.height);
                    }
                }
            ...
            w:Number):Number
        {
            ...
            for(i = 0; i < n; i++)
            {
                ...
                if(child is IUIComponent)
                {
                    ...
                    if(!isNaN(percentWidth) && IUIComponent(child).includeInLayout)
                    {
                        width = Math.max(IUIComponent(child).minWidth,
                                         Math.min(IUIComponent(child).maxWidth,
                                                  ((percentWidth >= 100) ? w : w * percentWidth / 100)));
                    }
                    else
                    {
                        width = IUIComponent(child).getExplicitOrMeasuredWidth();
                    }
                    ...
                    if(!isNaN(percentHeight) && IUIComponent(child).includeInLayout)
                    {
                        ...
                        childInfo.min = IUIComponent(child).minHeight;
                        childInfo.max = IUIComponent(child).maxHeight;
                    ...
                    {
                        var height:Number = IUIComponent(child).getExplicitOrMeasuredHeight();
                        // if scaled and zoom is playing, best to let the sizes be non-integer
                        // otherwise the rounding creates an error that accumulates in some components like List
                        if(child.scaleX == 1 && child.scaleY == 1)
                        {
            ...
            if(totalPercentHeight)
            {
                ...
                for(i = 0; i < n; i++)
                {
                    ...
                    if(child.scaleX == 1 && child.scaleY == 1)
                    {
                        IUIComponent(child).setActualSize(Math.floor(childInfo.width),
                                            Math.floor(childInfo.size));
                    }
                    else
                    {
                        IUIComponent(child).setActualSize(childInfo.width, childInfo.size);
                    }
                }
